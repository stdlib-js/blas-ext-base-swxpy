...
var swxpy = tryRequire( resolve( __dirname, './../lib/swxpy.native.js' ) );
var opts = {
	'skip': ( swxpy instanceof Error )
};
...
tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( swxpy.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	w = new Float32Array( x.length );
	out = swxpy( x.length, x, 1, y, 1, w, 1 );
...
tape( 'the function supports view offsets', opts, function test( t ) {
	...
	w1 = new Float32Array( w0.buffer, w0.BYTES_PER_ELEMENT*3 );
